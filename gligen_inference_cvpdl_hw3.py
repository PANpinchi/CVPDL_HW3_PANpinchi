...
@torch.no_grad()
def run(meta, config, model, autoencoder, text_encoder, diffusion, starting_noise=None):
    # - - - - - prepare batch - - - - - #
    if "keypoint" in meta["ckpt"]:
        ...
        batch = prepare_batch(meta, config.batch_size, max_objs=len(meta["locations"]))
        # batch.keys(): dict_keys(['boxes', 'masks', 'text_masks', 'image_masks', 'text_embeddings', 'image_embeddings'])
    ...
    for sample in samples_fake:
        img_name = os.path.join(output_folder, meta["save_file_name"])
        sample = torch.clamp(sample, min=-1, max=1) * 0.5 + 0.5
        sample = sample.cpu().numpy().transpose(1, 2, 0) * 255
        sample = Image.fromarray(sample.astype(np.uint8))
        sample.save(img_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_type",
        type=str,
        choices=["box_text", "box_text_image"],
        default="box_text",
    )
    parser.add_argument(
        "--prompt_type",
    ...
    for entry in tqdm(label_data):
        output_folder = os.path.join(args.folder, args.outdir)
        os.makedirs(output_folder, exist_ok=True)
        output_file = os.path.join(output_folder, entry["image"])
        ...
        if args.model_type == "box_text":
            meta = dict(
                ckpt=ckpt,
                prompt=entry[args.prompt_type],
                phrases=entry["labels"],
                locations=entry["bboxes"],
                alpha_type=[0.3, 0.0, 0.7],
                save_folder_name=args.outdir,
                save_file_name=entry["image"],
            )
        else:
            meta = dict(
                ckpt=ckpt,
                prompt=entry[args.prompt_type],
                images=['../images/{}'.format(entry["image"])],
                phrases=entry["labels"],
                locations=entry["bboxes"],
                alpha_type=[0.3, 0.0, 0.7],
                save_folder_name=args.outdir,
        ...
        run(meta, args, model, autoencoder, text_encoder, diffusion, starting_noise=None)
